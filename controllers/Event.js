...
export const createEvent = async (req, res, next) => {
    try {
        ...
        if (!title || !date || !startTime || !endTime || !type) {
            return res.status(400).json({ message: "Title, date, startTime, endTime, and type are required." });
        }
        ...
        if (!["online", "in-person"].includes(type)) {
            return res.status(400).json({ message: "Invalid event type. Must be 'online' or 'in-person'." });
        }
        ...
        if (type === "online") {
            if (!location) {
                ...
                return res.status(400).json({ message: "For online events, location must be a valid Jitsi link." });
            }
        }
...
export const getOrganizerUpcomingEvents = async (req, res, next) => {
    try {
        ...
        const events = await Event.find(query, "title date startTime endTime type status") // Project minimal fields
...
export const getOrganizerLiveEvents = async (req, res, next) => {
    try {
        ...
        if (liveEvents.length === 0) {
            return res.status(200).json({ message: "No live events found.", liveEvents: [] });
        }
...
export const getParticipantMyEvents = async (req, res, next) => {
    try {
        ...
        const myRSVPs = await RSVP.find({ participantId: req.user.id, status: "active" }).populate("eventId");
    ...
    try {
        ...
        if (req.user.role === "organizer") {
            ...
            if (!participantRSVP) {
                ...
                event.type === "online" &&
                currentDate < new Date(event.date + "T" + event.startTime).getTime() - 24 * 60 * 60 * 1000
            ) {
                ...
                response.location = "Please refer to your ticket for event location details.";
            }
    ...
    try {
        ...
        if (type) {
            ...
            if (!validTypes.includes(type)) {
                return res.status(400).json({ message: "Invalid event type. Allowed values are 'online' or 'in-person'." });
            }
            query.type = type;
        }
...
export const updateEvent = async (req, res, next) => {
    try {
        ...
        if (currentDateTime >= eventStartDateTime && currentDateTime <= eventEndDateTime) {
            return res.status(400).json({ message: "Cannot update an event that is currently live." });
        }
        if (currentDateTime > eventEndDateTime) {
            return res.status(400).json({ message: "Cannot update a completed event." });
        }
        ...
        if (req.file) {
            ...
            event.thumbnail = req.file.path.replace('/upload/', '/upload/w_300,h_200,c_fill/');
        }
        ...
        for (const rsvp of rsvps) {
            try {
                ...
                console.error(`Failed to fetch participant (ID: ${rsvp.participantId}):`, emailError);
            }
        }
        ...
        if (participantEmails.length > 0) {
            ...
            for (const rsvp of rsvps) {
                try {
                    ...
                    console.error(`Failed to send email to participant (ID: ${rsvp.participantId}):`, emailError.message);
                }
            }
        }

        res.status(200).json({ message: "Event updated successfully and participants notified.", event });
    } catch (error) {
